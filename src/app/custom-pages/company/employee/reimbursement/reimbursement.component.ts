  ...
  employeeuploadedImage: any = null;
   ...
   constructor(
    ...
    this.breadCrumbItems = [
      ...
      { label: "Reimbursement", active: true },
    ];
    this.initializeForm();
...
initializeForm() {
  ...
  getquatationData() {
    this.toggleSpinner(true);
    const url = `reimbursement?company_id=${this.company_id}`;
    this.api.getwithoutid(url).subscribe(
      (res: any) => {
        this.toggleSpinner(false);
        if (res && res.status) {
          this.quatationDataList = res.data.recent_requests || [];
          this.filterdata();
        } else {
          this.quatationData = [];
...
console.log(this.quatationDataModal, "quatationDataModal");
  ...
  filterdata() {
    ...
    if (this.term) {
      filteredData = filteredData.filter(
        (el: any) =>
          el.employee_name.toLowerCase().includes(this.term.toLowerCase()) ||
    ...
    if (this.selectedEmp && this.selectedEmp.length > 0) {
      filteredData = filteredData.filter((el: any) =>
        this.selectedEmp
          .map((d) => d.toLowerCase())
          .includes(el.employee_name.toLowerCase())
      );
