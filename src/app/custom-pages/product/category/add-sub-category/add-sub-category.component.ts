...
export class AddSubCategoryComponent implements OnInit {
  ...
  getSubCategoryData() {
    this.api.getwithoutid("product-sub-category").subscribe(
      (res: any) => {
        if (res && res.status) {
          this.subcategoryData = res.data || [];
          this.subcategoryDataList = res.data || [];
  ...
  createcategoryFormData(): FormData {
    ...
    formData.append("category_id", this.categorySec["category_id"].value);

    return formData;
  }

  updatecategory(team: any, formData: FormData, id: number) {
    this.api.put("product-sub-category", id, formData).subscribe(
  ...
  addcategory(team: any, formData: FormData) {
    this.api.post("product-sub-category", formData).subscribe(
      (res: any) => this.handlecategoryResponse(res, team),
      (error) => this.handleError(error)
    );
  }
  ...
  categorysetStatus(serviceId: number, event: Event) {
    ...
    this.api.put("product-sub-category", serviceId, formData).subscribe(
    ...
    if (id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.api.deleteWithId("product-sub-category", id).subscribe(
  ...
  filterdata() {
    if (this.term) {
      this.subcategoryData = this.subcategoryDataList.filter((el: any) => {
      ...
      this.subcategoryData = this.subcategoryDataList.slice(0, 5);
    }
    // noResultElement
  ...
  updateNoResultDisplay() {
    ...
    if (this.term && this.subcategoryData.length === 0) {
  ...
  updatePagination() {
    ...
    this.totalItems = this.subcategoryDataList.length;
  }
  updateDisplayedItems(): void {
    const startItem = (this.currentPage - 1) * this.itemsPerPage;
    this.endItem = this.currentPage * this.itemsPerPage;
    this.subcategoryData = this.subcategoryDataList.slice(
      startItem,
      this.endItem
    );
  }
  ...
  showError() {
    Swal.fire({
      title: "Looks like you haven't created a product category.",
      text: "Please add a product category.",
      icon: "warning",
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire("Cancelled", "Please add a category to proceed.", "info");
      }
    });
  }
}
