...
export class ServiceBannerComponent implements OnInit {
  ...
  getData() {
    ...
    this.api.getwithoutid("get-service-banner").subscribe(
      (res: any) => {
  ...
  addSection(formData: FormData) {
    this.api.post("insert-service-banner", formData).subscribe(
      (res: any) => {
        this.reset();
  ...
  updateSection(formData: FormData) {
    ...
    this.api.put("insert-service-banner", urlId, formData).subscribe(
