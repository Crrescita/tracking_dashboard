...
export class AddServiceComponent implements OnInit {
  ...
  // api's
  getData() {
    this.api.getwithoutid("service").subscribe(
      (res: any) => {
        if (res && res.status) {
  ...
  createFormData(): FormData {
    ...
    formData.append("layout_type", this.s2["layout_type"].value);
    formData.append("position", this.s2["position"].value);
    ...
    if (id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.api.deleteWithId("service", id).subscribe(
  ...
  setStatus(serviceId: number, event: Event) {
    ...
    this.api.put("service", serviceId, formData).subscribe(
      (res: any) => this.handleStatusChangeResponse(res),
      (error) => this.handleError(error)
    );
  }
